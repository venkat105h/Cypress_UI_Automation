...
//SWAT Jan 2022
  it.only('Search attendees on live stream for session with all params: POST /api/v2/sessions/attendees', async () => {
    ...
    expect(response.body.message).to.equal(Responsemessages.Parameter_session_attendee_list_get_message);
    expect(response.body.success.data.length).to.equal(1);
    expect(response.body.success.data.attendees[0].firstName).to.equal("Joker");
    expect(response.body.success.data.attendees[0].lastName).to.equal("Clown");
  });
...
it.only('Create a session to check Pre Recorded video streaming : POST /api/v1/session/upload/video', async () => {
  ...
  it.only('login to admin dashboard: POST /backend/api/v2/auth/login', async () => {
    ...
    var response = await sendRequest(environment.baseURL7, ApiEndPoints.Adminloginurl, { 'Content-Type': 'application/json' }, 'post', login1)
...
it.only('Delete Session : POST /backend/api/v2/agendas/delete', async () => {
  ...
  it.only('Create a session for next day : POST /backend/api/v2/events/agendas', async () => {
    var session = new Session();
     var sessionEndTime = (addTime(new Date(), 25)).getTime()
     var sessionStartTime = (addTime(new Date(), 24)).getTime()
...
it.only('Add a speaker and sign', async () => {
  ...
  var people = new People();
  global.peopleId_speaker = await people.addSingleAttendeeAndVerify(organizerUserHeader(), process.env.eventid, 'speakerfilteruser22@yopmail.com', 'speakerseesion22', 'user', [global.speakergroup])
});
